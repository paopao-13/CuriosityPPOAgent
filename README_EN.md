...
| MiniGrid DoorKey (pure curiosity, 16×16) | 242万步 convergence | **0.0** (1.5M steps) | 96.8万步 (success≥0.95) | success_rate, DoorKey unsolved; root cause = equal-weight advantage merge drowns extrinsic signal |
| MiniGrid DoorKey (curriculum, 8×8) | — | **0.21** (~2M steps: fixed-layout warmup 500K + random generalization 1.5M) | 96.8万步 (success≥0.95) | reward shaping + ext advantage weight (ext_adv_coef=4); not pure-curiosity |
| MiniGrid DoorKey (potential shaping, 8×8) | — | **1.00** (4M steps, success_rate greedy 100-ep) | 96.8万步 (success≥0.95) | potential-based shaping Φ=−Manhattan dist to subgoal gives dense guidance; ext_adv_coef=2; not pure-curiosity |

> † Our Crafter score (0.2%) is the 22-achievement geometric mean under a **pure-curiosity setup (no extrinsic reward shaping)**, taken from training-time auto-eval (`results/ablation/crafter_full/seed_42/train.log`, step=1000448). The PPO baseline 15.6% is standard PPO(ResNet) *with* extrinsic reward — different training conditions, not directly comparable as a win/loss. MiniGrid's three rows are all locally reproducible: 0.0 from the pure-curiosity baseline (`results/ablation/minigrid_doorkey_full/seed_42/train.log`, step=1501184); 0.21 from curriculum learning (`results/ablation/minigrid_curriculum/phase2/seed_42/_wrapper.log`); 1.00 from potential shaping (`results/ablation/minigrid_potential/seed_42/_wrapper.log`, tail eval stable at 1.00). Note that 0.21 and 1.00 are **not pure-curiosity setups** (reward shaping + ext advantage weighting) — the pure-curiosity baseline remains 0.0. This is precisely the project's key finding: the root cause was "equal-weight advantage merge drowns extrinsic signal", not the curiosity modules themselves. All measured scores reproducible via `scripts/evaluate.py --env crafter/minigrid`; model weights excluded from the repo (size).
